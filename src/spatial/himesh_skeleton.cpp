...
vector<Voxel *> HiMesh::generate_voxels_skeleton(int voxel_size){
	...
	// @@ we need to adjust core of voxel.

	std::vector<std::vector<Point>> voxel_points(voxels.size());
	for (Facet_const_iterator f = facets_begin(); f != facets_end(); ++f) {
		Point p = HiMesh::barycenter(f);
		aab box = HiMesh::bounding_box(f);
		...
		for (int j = 0; j < (int)skeleton_points.size(); j++) {
			float cur_dist = distance(p, skeleton_points[j]);
			if (cur_dist < min_dist) {
				gid = j;
				min_dist = cur_dist;
		...
		voxel_points[gid].push_back(p);
	}


	for (int i = 0; i < (int)voxels.size(); ++i) {
		if (voxel_points[i].empty()) continue;

		auto* v = voxels[i];

		float best_dist = DBL_MAX;
		float best_point[3];
		best_point[0] = v->core[0];
		best_point[1] = v->core[1];
		best_point[2] = v->core[2];

		for (const auto& p : voxel_points[i]) {
			float dx = p[0] - v->core[0];
			float dy = p[1] - v->core[1];
			float dz = p[2] - v->core[2];
		...
		v->core[0] = best_point[0];
		v->core[1] = best_point[1];
		v->core[2] = best_point[2];
	}
