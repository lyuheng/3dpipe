...
void load_prototype(const char *nuclei_path, const char *vessel_path){
	...
	// float max_side_vessel = max({mbb.high[0] - mbb.low[0],
	// 						mbb.high[1] - mbb.low[1],
	// 						mbb.high[2] - mbb.low[2]});

    // vessel->shrink(max_side_vessel/10);
	...
	// float max_side_nuclei = max({mbb.high[0] - mbb.low[0],
	// 							mbb.high[1] - mbb.low[1],
	// 							mbb.high[2] - mbb.low[2]});
    // nuclei->shrink(max_side_nuclei);
	...
	vector<Voxel *> vessel_voxels = vessel->generate_voxels_skeleton(40);
		...
		while (idx < total_slots) {
			...
			if (skipped++ == gap) {
				...
				if (multi_lods) {
					...
					wr->find_closest_mbb_point(nuclei_inside);
				}
...
void *generate_unit(void *arg){
	...
	while(true){
		...
		if(multi_lods){
			...
			wr->find_closest_mbb_point(vessel_inside);
		}
		pthread_mutex_lock(&mylock);
...
int main(int argc, char **argv){
	...
	nuclei_inside = inside_polyhedron(nuclei_pt);
	vessel_inside = inside_polyhedron(vessel_pt);
	...
	ofstream *nuclei_os = new std::ofstream(nuclei_output, std::ios::out | std::ios::binary);
	...
	for(int i=0;i<x_dim;i++){
		for(int j=0;j<y_dim;j++){
			for(int k=0;k<z_dim;k++){
				...
				if (jobs.size() == JOB_THRESHOLD ||
					(i==x_dim-1 && j==y_dim-1 && k==z_dim-1)) {
					...
					if(multi_lods){
						...
						nuclei_tile->dump_compressed(nuclei_output, nuclei_os);
