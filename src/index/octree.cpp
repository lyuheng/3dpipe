...
void OctreeNode::query_knn(weighted_aab *box, float *transform, vector<pair<int, range>> &candidates, float &min_maxdist, const int k){
	...
	if(dis.mindist<min_maxdist || candidates.size()<k){
		if(isLeaf){
			for(weighted_aab *obj:objectList){
				...
				objdis.maxdist = sqrt(
								(transform[0] - obj->transform[0]) * (transform[0] - obj->transform[0]) +
								(transform[1] - obj->transform[1]) * (transform[1] - obj->transform[1]) +
								(transform[2] - obj->transform[2]) * (transform[2] - obj->transform[2])
							);
			...
			for(OctreeNode *c:this->children){
				c->query_knn(box, transform, candidates, min_maxdist, k);
			}
		}
	}
}

void OctreeNode::query_knn_fast(weighted_aab* box,
							float *transform,
							std::vector<std::pair<int, range>>& candidates,
							float &min_maxdist,
							int k)
{
    ...
    std::priority_queue<NodeEntry,
                        ...
                        std::greater<NodeEntry>> nodeHeap;

    range rootRange = this->distance(*box);
    nodeHeap.push({this, rootRange.mindist});
    ...
    while (!nodeHeap.empty()) {
        ...
        if (node->isLeaf) {
            ...
            for (weighted_aab* obj : node->objectList) {

                if (obj == box) continue;

                range r = obj->distance(*box);
				r.maxdist = sqrt(
								(transform[0] - obj->transform[0]) * (transform[0] - obj->transform[0]) +
								(transform[1] - obj->transform[1]) * (transform[1] - obj->transform[1]) +
								(transform[2] - obj->transform[2]) * (transform[2] - obj->transform[2])
				...
				if(add_candidate(candidates, obj->id, r, k) && candidates.size() >= k){
					upperBound = candidates[k-1].second.maxdist;
				}
            }
            ...
            //             return a.second.maxdist < b.second.maxdist;
            //         });
            ...
            for (OctreeNode* child : node->children) {
                ...
                range r = child->distance(*box);

                if (r.mindist <= upperBound) {
                    nodeHeap.push({child, r.mindist});
                }
            }
        }
    }
...
void OctreeNode::query_within(weighted_aab *box, float *transform, vector<pair<int, range>> &results, const float threshold){
	range dis = distance(*box);
	...
	if(dis.mindist<=threshold){
		if(isLeaf){
			...
			for(OctreeNode *c:this->children){
				c->query_within(box, transform, results, threshold);
			}
		}
	}
}
