...
void geometry_computer::initialize_refine_stream() {
	tdbase::initialize_refine_stream();
}
...
void geometry_computer::get_distance_gpu(geometry_param &cc){
	...
	// tdbase::MeshDist_batch_gpu_op(gpu, cc.data, cc.offset_size, cc.hausdorff, cc.results, cc.pair_num, cc.element_num);
	// tdbase::MeshDist_batch_gpu_op_shm(gpu, cc.data, cc.offset_size, cc.hausdorff, cc.results, cc.pair_num, cc.element_num);
	tdbase::MeshDist_batch_gpu_op_shm_flat(gpu, 
										cc.data, 
										cc.offset_size, 
										cc.hausdorff, 
										cc.results, 
										...
										cc.confirmed_after,
										cc.status_after,
										cc.valid_voxel_prefix,
										cc.valid_voxel_pairs,
										cc.lod,
										cc.total_voxel_size,
										cc.min_min_dist,
										cc.min_max_dist
...
void geometry_computer::get_distance_gpu_stream(geometry_param &cc){
	gpu_info *gpu = request_gpu(cc.element_num*11*sizeof(float)/1024/1024, true);
	// assert(gpu);
	log("GPU %d started to get distance", gpu->device_id);
	// tdbase::MeshDist_batch_gpu(gpu, cc.data, cc.offset_size, cc.hausdorff, cc.results, cc.pair_num, cc.element_num);
	// tdbase::MeshDist_batch_gpu_op(gpu, cc.data, cc.offset_size, cc.hausdorff, cc.results, cc.pair_num, cc.element_num);
	// tdbase::MeshDist_batch_gpu_op_shm(gpu, cc.data, cc.offset_size, cc.hausdorff, cc.results, cc.pair_num, cc.element_num);
	tdbase::MeshDist_batch_gpu_op_shm_flat_stream(gpu, 
										cc.data, 
										cc.offset_size, 
										cc.hausdorff, 
										cc.results,
...
void geometry_computer::get_distance(geometry_param &cc){
	if(gpus.size()>0){
#ifdef USE_GPU
		std::cout << "LAUNCH GPU KERNEL" << std::endl;
		get_distance_gpu(cc);
#endif
	}else{
		get_distance_cpu(cc);
