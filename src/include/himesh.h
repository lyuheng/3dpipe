...
public:
	...
	void find_closest_mbb_point(bool inside) {

		if (inside) {
			transform[0] = 0.5f * (mesh->get_mbb().low[0] + mesh->get_mbb().high[0]);
			transform[1] = 0.5f * (mesh->get_mbb().low[1] + mesh->get_mbb().high[1]);
			transform[2] = 0.5f * (mesh->get_mbb().low[2] + mesh->get_mbb().high[2]);
		}
		else {
		
			Point *vertices = NULL;
			const size_t numv = mesh->fill_vertices((float *&)vertices);

			aab mbb = mesh->get_mbb();
			Point center(
				0.5f * (mesh->get_mbb().low[0] + mesh->get_mbb().high[0]),
				0.5f * (mesh->get_mbb().low[1] + mesh->get_mbb().high[1]),
				0.5f * (mesh->get_mbb().low[2] + mesh->get_mbb().high[2])
			);
			...
			for (size_t i = 0; i < numv; ++i) {
				const float dx = vertices[i].x() - center.x();
				const float dy = vertices[i].y() - center.y();
				const float dz = vertices[i].z() - center.z();
			...
			transform[0] = vertices[best_idx][0];
			transform[1] = vertices[best_idx][1];
			transform[2] = vertices[best_idx][2];
